...
# Global variables
TARGET=""
OUTPUT_DIR="/root/bug-hunting"
TOOL_CONFIG="$HOME/.bug_hunter_config"
SESSION_FILE="/root/bug-hunting"
...
utilities_view_progress() {
    ...
    if [ -f "$SESSION_FILE" ]; then
        ...
        # Count findings - LINE INI YANG DIPERBAIKI
        echo ""
        ...
        if [ -d "$OUTPUT_DIR/tests" ]; then
            for test_dir in "$OUTPUT_DIR/tests"/*/; do
                if [ -d "$test_dir" ]; then
                    ...
                    if ls "$test_dir"/*.txt 1>/dev/null 2>&1; then
                        count=$(grep -l -i "vulnerable\|issue\|finding\|positive" "$test_dir"/*.txt 2>/dev/null | wc -l)
                    fi
                    if [ $count -gt 0 ]; then
